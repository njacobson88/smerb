...
def email_pdf_to_slack(pdf_path, assessment, generated_by, logger):
    """Send the PDF to the Slack channel via Microsoft Graph email."""
    try:
        from graph_email import send_graph_email, graph_email_configured, GRAPH_SENDER

        slack_email = os.getenv("SLACK_CHANNEL_EMAIL")
        sender = os.getenv("ALERT_SENDER_EMAIL", GRAPH_SENDER)

        if not graph_email_configured() or not slack_email:
            logger.warning("[RiskAssessment] Graph email/Slack not configured")
            return False
        ...
        send_graph_email(
            slack_email,
            f"[{assessment['riskLevel']} RISK] Risk Assessment — Participant {pid}",
            text=(
            ...
            sender=sender,
            attachments=[{
                "name": f"risk_assessment_{pid}.{'pdf' if is_pdf else 'html'}",
